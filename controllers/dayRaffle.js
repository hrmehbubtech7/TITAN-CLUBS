...
const DayHash = require('../models/DayHash');
const Profit = require("../models/Profit");
const { generateFloats, sha256 } = require('../functions/game');
const total_time = 3600 * 24 * 1000;
...
const total_win_tickets = Number(process.env.DAILY_1_COUNT) + Number(process.env.DAILY_2_COUNT) + Number(process.env.DAILY_3_COUNT) + Number(process.env.DAILY_5_COUNT) + Number(process.env.DAILY_4_COUNT);

const finishing_period = async () => {
	setTimeout(betting_period, finishing_time);
	let bets = await DayBet.find({});
	if (bets.length > 0) {
		const tmp_profits = {};
		tmp_profits.period = bets[0].no;
		tmp_profits.raffle = "Daily";
		tmp_profits.amount = 0;
		for (let bet of bets) {
			tmp_profits.amount += Number(process.env.DAILY_PRICE);
			const place = await DayTicket.find({ ticket: bet.ticket });
			const financial = {};
			financial.type = "Daily";
			financial.amount = -Number(process.env.DAILY_PRICE);
			financial.details = {};
			...
			if (place.length > 0) {
				await User.findByIdAndUpdate(bet.userid, {
					$inc: {
						...
						'prize': Number(place[0].prize) - Number(process.env.DAILY_PRICE)
					}
				});
			}
		}
	...
	const places = await DayTicket.find({});
	for (let place of places) {
		...
		bets = await DayBet.find({ ticket: place.ticket });
		if (bets.length > 0) {
			tmp_result.userid = bets[0].userid;
		}
		await (new DayResult(tmp_result)).save();
	}
	await DayHash.updateOne({ nonce: period }, { used: true });
	await DayBet.deleteMany({});
	await DayTicket.deleteMany();
};
const betting_period = async () => {
	setTimeout(finishing_period, total_time - finishing_time);
	d = new Date();
	start_time = d.getTime();
	const priorBets = await DayBet.find({});
	for (let i = 0; i < priorBets.length; i++) {
		await User.findByIdAndUpdate(priorBets[i].userid, {
			$inc: {
				'budget': Number(process.env.DAILY_PRICE),
				'bets': -Number(process.env.DAILY_PRICE)
			}
		});
	}
	await DayBet.deleteMany({});
	await DayTicket.deleteMany({});
	const lastNo = await DayHash.find({ used: false }).sort({ 'nonce': 1 }).limit(1);
	...
	let tmp_i = 0;
	let tmp_no;
	for (let i = 0; i < Number(process.env.DAILY_1_COUNT); i++) {
		tmp_no = Math.floor(randomNumbers[tmp_i] * tmp_tickets.length);
		await DayTicket.create({
			no: period,
			ticket: tmp_tickets[tmp_no],
			place: 1,
			prize: Number(process.env.DAILY_1_PRIZE)
		});
		tmp_tickets.splice(tmp_no, 1);
		tmp_i++;
	}
	for (let i = 0; i < Number(process.env.DAILY_2_COUNT); i++) {
		tmp_no = Math.floor(randomNumbers[tmp_i] * tmp_tickets.length);
		await DayTicket.create({
			no: period,
			ticket: tmp_tickets[tmp_no],
			place: 2,
			prize: Number(process.env.DAILY_2_PRIZE)
		});
		tmp_tickets.splice(tmp_no, 1);
		tmp_i++;
	}
	for (let i = 0; i < Number(process.env.DAILY_3_COUNT); i++) {
		tmp_no = Math.floor(randomNumbers[tmp_i] * tmp_tickets.length);
		await DayTicket.create({
			no: period,
			ticket: tmp_tickets[tmp_no],
			place: 3,
			prize: Number(process.env.DAILY_3_PRIZE)
		});
		tmp_tickets.splice(tmp_no, 1);
		tmp_i++;
	}
	for (let i = 0; i < Number(process.env.DAILY_4_COUNT); i++) {
		tmp_no = Math.floor(randomNumbers[tmp_i] * tmp_tickets.length);
		await DayTicket.create({
			no: period,
			ticket: tmp_tickets[tmp_no],
			place: 4,
			prize: Number(process.env.DAILY_4_PRIZE)
		});
		tmp_tickets.splice(tmp_no, 1);
		tmp_i++;
	}
	for (let i = 0; i < Number(process.env.DAILY_5_COUNT); i++) {
		tmp_no = Math.floor(randomNumbers[tmp_i] * tmp_tickets.length);
		await DayTicket.create({
			no: period,
			ticket: tmp_tickets[tmp_no],
			place: 5,
			prize: Number(process.env.DAILY_5_PRIZE)
		});
		tmp_tickets.splice(tmp_no, 1);
		tmp_i++;
	}
...
exports.getRaffle = async (req, res, next) => {
	try {
		const soldTickets = await DayBet.find({}).populate('userid');
...
exports.postRaffle = async (req, res, next) => {
	try {
		...
		const ticket = Number(req.body.third + "" + req.body.second + "" + req.body.first);
		const isExisted = await DayBet.countDocuments({ ticket: ticket });
		if (isExisted > 0) {
		...
		if (Number(user.budget) < Number(process.env.DAILY_PRICE)) {
		...
		const current = await DayTicket.findOne({});
		const tmp = {};
		tmp.userid = user._id;
		tmp.ticket = ticket;
		tmp.no = period;
		await (new DayBet(tmp)).save();
		const soldTickets = await DayBet.find({}).populate('userid');
		return res.status(200).json({
			time: (total_time - (cur_time - start_time)) / 1000,
			soldTickets, period, hash,
