...
exports.top_users = async (req, res, next) => {
  const seniors = await User.find({}).sort({ 'prize': -1 }).limit(5);
  ...
  const weeklyResult1 = await WeekResult.find({ place: 1 }).populate('userid').sort({ no: -1 }).limit(1);
  const weeklyResult2 = await WeekResult.find({ place: 2 }).populate('userid').sort({ no: -1 }).limit(Number(process.env.WEEKLY_2_COUNT));
  const dailyResult1 = await DayResult.find({ place: 1 }).populate('userid').sort({ no: -1 }).limit(1);
  const dailyResult2 = await DayResult.find({ place: 2 }).populate('userid').sort({ no: -1 }).limit(Number(process.env.DAILY_2_COUNT));
  const hourlyResult1 = await HourResult.find({ place: 1 }).populate('userid').sort({ no: -1 }).limit(1);
  const hourlyResult2 = await HourResult.find({ place: 2 }).populate('userid').sort({ no: -1 }).limit(Number(process.env.HOURLY_2_COUNT));

  const reviews = await Review.find({}).populate('userid').sort({ createdAt: -1 }).limit(20);
  return res.status(200).json({
    seniors, maxBets, weeks: [...weeklyResult1, ...weeklyResult2],
    days: [...dailyResult1, ...dailyResult2], hours: [...hourlyResult1, ...hourlyResult2], reviews
  });
};
...
exports.getRecords = async (req, res, next) => {
  ...
  try {
    ...
    if (req.params.game == 0) {
      //hourly
      const tmp_my_records = user.financials.filter((ele)=>ele.type=="Weekly");
      const last = tmp_my_records.length;
      let my_records = tmp_my_records.slice((page-1)*10, page*10);     
      return res.status(200).json({
        my_records,
        records_my_page: page,
        last_records_my_page:Math.ceil(last / 10)
      });
    }else if (req.params.game == 1) {
      //hourly
      const tmp_my_records = user.financials.filter((ele)=>ele.type=="Daily");
      const last = tmp_my_records.length;
      let my_records = tmp_my_records.slice((page-1)*10, page*10);    
      return res.status(200).json({
        my_records,
    ...
    }else if (req.params.game == 2) {
      //hourly
      const tmp_my_records = user.financials.filter((ele)=>ele.type=="Hourly");
      const last = tmp_my_records.length;
      let my_records = tmp_my_records.slice((page-1)*10, page*10);
      return res.status(200).json({
        my_records,
    ...
    }else if (req.params.game == 3) {
      //color
      let reviews, enjoy_count;
      reviews = await MyEnjoy.find({ user: req.userFromToken._id }).sort({ _id: -1 }).skip((page - 1) * 10).limit(10);
      enjoy_count = await MyEnjoy.countDocuments({ user: req.userFromToken._id });
      const my_records = [];
      for (review of reviews) {
        const tmp = {};
        tmp.period = review.period;
        tmp.contract = review.contract;
        tmp.select = review.select;
        tmp.result = review.result;
        tmp.amount = review.amount;
        my_records.push(tmp);
      }
      return res.status(200).json({
        my_records,
      ...
      //toss
      let reviews, enjoy_count;
      reviews = await Toss.find({ userid: req.userFromToken._id }).sort({ _id: -1 }).skip((page - 1) * 10).limit(10);
      enjoy_count = await Toss.countDocuments({ userid: req.userFromToken._id });
      const my_records = [];
      for (review of reviews) {
        const tmp = {};
        tmp.period = review.createdAt;
        tmp.contract = review.bet ;
        tmp.select = '';
        tmp.result = '';
        tmp.amount = review.profit ;
        my_records.push(tmp);
      }
      return res.status(200).json({
        my_records,
