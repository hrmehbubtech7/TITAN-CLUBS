...
class TossPage extends React.Component {
  ...
  copySeed = () => {
    ...
    tmp_tag.value = this.state.server_seed;
    document.body.appendChild(tmp_tag);
    tmp_tag.select();
    document.execCommand("copy");
    document.body.removeChild(tmp_tag);
    this.setState({ successMessage: "Server Seed is copied." });
  };
          ...
          response.json().then(res => {
            ...
            if (res.error) {
              ...
              setTimeout(() => {
                ...
                if (res.profit < 0) {
                  this.setter.setState({ errorMessage: "You lost ₹" + (-res.profit) });
                } else {
                  this.setter.setState({ successMessage: "You got ₹" + res.profit });
                }
                if(res.levelup>0){
      ...
      return (
          ...
          breadcrumbs={
            (
              <div className="header-buttons">
                ...
                <Button component="a" size="sm" style={{ "float": "right", marginTop: "-30px" }}
                  color="info" onClick={() => this.setState({ fairness_modal: true })}>
                  Fairness
                </Button>
              </div>
            )
        ...
        >
              ...
              <Card className="mb-3">
                ...
                <CardBody >
                  ...
                  <Modal
                    ...
                    className={this.props.className}>
                    <ModalHeader toggle={() => this.setState({ fairness_modal: !this.state.fairness_modal })}>Fairness</ModalHeader>
                    <ModalBody>
                      <Row>
                        <Col xs={12} lg={12}>
                          <div>
                            The Previous Server Seed :
                        </div>
                          <div style={{ overflow: 'auto' }}>
                            {this.state.server_seed}
                          </div>
                        </Col>
                        <Col xs={12} lg={12} className="mt-2">
                          <div>
                            The Current Nonce : {this.state.tails_count + this.state.heads_count}
                          </div>
                          <div>
                            The Current Hash :
                        </div>
                    ...
                    <ModalFooter>
                      ...
                      <Button color="primary" onClick={() => window.open("/coinflip-fairness.htm")}>
                        Verify
                          </Button>{' '}
          ...
          <Snackbar
            ...
            open={this.state.errorMessage!=false}
